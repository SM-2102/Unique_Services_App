...
const MarketCreatePage = () => {
  ...
  useEffect(() => {
    let mounted = true;
    fetchNextMarketNumber()
      .then((data) => {
        if (mounted && data) {
          setForm((prev) => ({
            ...prev,
            receipt_number: data.next_mcode || "",
          }));
        }
      })
      .catch(() => {
  ...
  // Validation
  const [errs, errs_label] = validateMarketCreate(form);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError({
        message: err?.message || "Failed to create market record.",
    ...
    <div className="flex min-h-[80vh] mt-6 justify-center items-center">
      ...
      >
        ...
        <div className="flex flex-col gap-4">
          {/* Market Receipt Number (readonly, small, label beside input) */}
          ...
          <div className="flex items-center w-full gap-4">
            <div className="flex items-center w-1/2 gap-2">
              ...
              <input
                ...
                onChange={handleChange}
                className={`w-full px-3 py-1 rounded-lg border ${errs_label.invoice_number ? "border-red-300" : "border-gray-300"} bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400 font-small`}
                maxLength={10}
                disabled={submitting}
              />
            </div>
          ...
          {/* Quantity - label beside input, w-1/2 each */}
          <div className="flex items-center w-full">
            <div className="flex items-center w-1/2 gap-2">
              <label
                htmlFor="quantity"
                className="w-160 text-md font-medium text-gray-700"
              >
                No. of Products Received<span className="text-red-500">*</span>
              </label>
              <input
                id="quantity"
                name="quantity"
                ...
                value={form.quantity}
                onChange={handleChange}
                className={`w-full mr-4 px-3 py-1 rounded-lg border ${errs_label.quantity ? "border-red-300" : "border-gray-300"} bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-400 font-small`}
                min={1}
                disabled={submitting}
              />
            </div>
