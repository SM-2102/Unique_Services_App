...
import { fetchLastWarrantyCNFChallanCode } from "../services/warrantyCNFChallanLastCodeService";
import { printWarrantyCNF } from "../services/warrantyCNFChallanPrintService";

const WarrantyCNFPrintPage = () => {
  ...
  useEffect(() => {
    async function loadLastChallanNumber() {
      try {
        const data = await fetchLastWarrantyCNFChallanCode();
        setChallanNumber(data.last_cnf_challan_code || "");
      } catch (err) {
        setError({
          message: err.message,
          ...
          <div className="flex items-center gap-2">
            ...
            <IconButton
              ...
              onClick={async (e) => {
                ...
                try {
                  const blob = await printWarrantyCNF(challanNumber);
                  ...
                  if (newTab) {
                    newTab.document.write(
                      `<html><head><title>Warranty CNF Preview</title></head><body style='margin:0'>` +
                        `<iframe id='warranty-cnf-pdf-frame' src='${url}' width='100%' height='100%' style='border:none;min-height:100vh;'></iframe>` +
                        ...
                        var frame = document.getElementById('warranty-cnf-pdf-frame');
                  ...
                  setError({
                    message: err.message || "Failed to print warranty CNF.",
                    resolution: err.resolution || "",
                    type: "error",
                  });
                  setShowToast(true);
...
export default WarrantyCNFPrintPage;
